...
describe.skipIf(isStubbed || process.env.SKIP_BUNDLE_SIZE === 'true' || process.env.ECOSYSTEM_CI)('minimal nuxt application', () => {
  ...
  it('default server bundle size', async () => {
    ...
    expect.soft(roundToKilobytes(modules.totalBytes)).toMatchInlineSnapshot(`"1545k"`)
  ...
  it('default server bundle size (inlined vue modules)', async () => {
    ...
    const serverStats = await analyzeSizes(['**/*.mjs', '!node_modules'], serverDir, rootDir)
    expect.soft(roundToKilobytes(serverStats.totalBytes)).toMatchInlineSnapshot(`"582k"`)

    const modules = await analyzeSizes(['node_modules/**/*'], serverDir, rootDir)
    expect.soft(roundToKilobytes(modules.totalBytes)).toMatchInlineSnapshot(`"198k"`)
  ...
  it('default server bundle size (pages)', async () => {
    ...
    expect.soft(roundToKilobytes(serverStats.totalBytes)).toMatchInlineSnapshot(`"308k"`)

    const modules = await analyzeSizes(['node_modules/**/*'], serverDir, pagesRootDir)
    expect.soft(roundToKilobytes(modules.totalBytes)).toMatchInlineSnapshot(`"1556k"`)
