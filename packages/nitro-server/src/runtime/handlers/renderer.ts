...
async function renderRoute (event: H3Event, ssrError?: (NuxtPayload['error'] & { url: string })): Promise<ReadableStream<Uint8Array> | Response> {
  ...
  if (!NO_SCRIPTS) {
    ...
    const dependencyOptions = ssrContext['~lazyHydratedModules']?.size
      ...
      : undefined
    ssrContext.head.push({
      link: getPreloadLinks(ssrContext, renderer.rendererContext, dependencyOptions).filter(l => !l.href.includes('virtual:vsc:')) as Link[],
    })
    ssrContext.head.push({
      link: getPrefetchLinks(ssrContext, renderer.rendererContext, dependencyOptions).filter(l => !l.href.includes('virtual:vsc:')) as Link[],
    })
    // 5. Payloads
    ssrContext.head.push({
