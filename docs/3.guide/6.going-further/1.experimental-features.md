...
::note
**Component islands are compatible with streaming.** Islands are rendered from a serialized component tree (powered by vue-onigiri) rather than stitched into the HTML in a post-render pass, so streaming needs no special handling for island content: there are no island anchors or teleports to relocate in the streamed body.
::
