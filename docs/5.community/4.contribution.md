...
* Capitalize headings following APA title case. In short: capitalize the first word, the last word, and all major words (nouns, verbs, adjectives, adverbs, pronouns), as well as any word of four letters or more. When in doubt, [capitalizemytitle.com](https://capitalizemytitle.com/style/APA) can help.
...
* Terminal commands: `npx nuxi init`
...
* When linking to external resources, link to the final URL rather than one that redirects. You can check with:
  ...
  A `200` status with an empty redirect URL means the link is fine. A `301` or `308` means you should link to the redirect URL instead.
