...
When rendering a server-only or island component, `<NuxtIsland>` makes a fetch request which comes back with a `NuxtIslandResponse` containing the island's serialized component tree (powered by [vue-onigiri](https://github.com/huang-julien/vue-onigiri)) and its head entries. (This is an internal request if rendered on the server, or a request that you can see in the network tab if it's rendering on client-side navigation.)
