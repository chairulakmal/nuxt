...
When rendering an island component, the island is rendered on the server and returned as a serialized component tree (powered by [vue-onigiri](https://github.com/huang-julien/vue-onigiri)) rather than pre-rendered HTML. `<NuxtIsland>` deserializes and renders this tree client-side, so the island component's own JS is never downloaded to the browser.
